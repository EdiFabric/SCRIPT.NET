# EdiFabric C# .NET Examples for NCPDP SCRIPT

**EdiFabric 11.0.0** is a .NET SDK that parses, generates, validates, and splits EDI files. These examples cover **NCPDP SCRIPT 10.6**.
...
| Path | Purpose |
| --- | --- |
| `NET 6/EdiFabric.Examples.NCPDP.Script.sln` | .NET 6 solution |
| `NET Framework 4.8/EdiFabric.Examples.NCPDP.Script.sln` | .NET Framework 4.8 solution |
| `NET Framework 4.8/EdiFabric.Examples.NCPDP.Script.Common/Config.cs` | Serial key shared by every example |
| `NET Framework 4.8/EdiFabric.Examples.NCPDP.Script.Demo/Program.cs` | Runnable walkthrough: read, then validate |
| `Files/` | Sample NCPDP SCRIPT messages |

## Requirements

- Visual Studio 2022, or the .NET SDK. [Download Visual Studio](https://visualstudio.microsoft.com/downloads/).
- .NET 6 for `NET 6/EdiFabric.Examples.NCPDP.Script.sln`. The projects set `<TargetFramework>net6.0</TargetFramework>` so they stay compatible with existing .NET 6 apps. EdiFabric 11.0.0 also provides `net8.0`, `net9.0`, and `net10.0`. To evaluate a later version, change that property (for example to `net8.0`) and rebuild.
- .NET Framework 4.8 for `NET Framework 4.8/EdiFabric.Examples.NCPDP.Script.sln`.
...
2. Paste that serial into `TrialSerialKey` in `NET Framework 4.8/EdiFabric.Examples.NCPDP.Script.Common/Config.cs`. The .NET 6 projects link this file, so one edit covers both solutions.

NuGet restore pulls **EdiFabric 11.0.0** and **EdiFabric.Templates.Ncpdp 3.0.0**.

## Getting started

**Sign up free for Community** at [edifabric.com/pricing](https://www.edifabric.com/pricing.html) and put your serial in `Config.TrialSerialKey`. Then open a solution, set **EdiFabric.Examples.NCPDP.Script.Demo** as the startup project, and run it.

From the command line:

```bash
cd "NET 6/EdiFabric.Examples.NCPDP.Script.Demo"
dotnet run
```

The demo reads `Files/PrescriptionRequest_NEWRX.txt`, parses every message with `NcpdpScriptReader`, and validates each one with `IsValid`. Set a breakpoint at the end of `Translate_NCPDP_106` and inspect `ediItems`.

To translate your own file, change the path in `EdiFabric.Examples.NCPDP.Script.Demo/Program.cs`.
...
var ncpdpStream = File.OpenRead(@"Files\PrescriptionRequest_NEWRX.txt");
...
`NcpdpScriptReader` takes the stream and the template assembly name. `ReadToEnd` loads the file into memory. For large files, use the streaming samples in **ReadNCPDP**.

### Validation

After a message parses, `IsValid` checks it against the template. **ValidateNCPDP** shows custom codes, data types, and the transmission header.
...
### Writing NCPDP SCRIPT

**WriteNCPDP** builds a message with `NcpdpScriptWriter`: the interchange header, then the prescription request. The same project covers custom delimiters, a postfix after each segment, batches, empty data elements, and writing without auto trailers.
...
| --- | --- |
| `EdiFabric.Examples.NCPDP.Script.Demo` | Read a NEWRX prescription request and validate it |
| `EdiFabric.Examples.NCPDP.Script.ReadNCPDP` | Read to end, stream, batch, split on a repeating loop, corrupt files, partner templates, transaction only, custom UIB |
| `EdiFabric.Examples.NCPDP.Script.WriteNCPDP` | Write to a stream or file, delimiters, new lines, batches, empty elements, no auto trailers |
| `EdiFabric.Examples.NCPDP.Script.ValidateNCPDP` | Validate messages, custom codes, data types, the transmission header, and before write |
| `EdiFabric.Examples.NCPDP.Script.JSON` | Serialize and deserialize JSON |
| `EdiFabric.Examples.NCPDP.Script.XML` | `XmlSerializer` and `DataContractSerializer` |
...
The examples call `License.SetSerial(Config.TrialSerialKey)`. On Developer, call `License.EnsureToken` instead. `TokenFileCache.Set()` in `EdiFabric.Examples.NCPDP.Script.Common` is the manual `GetToken` / `SetToken` cache, for when you want to store the token yourself.
...
The same classes validate as well as parse. EdiFabric supports the NCPDP SCRIPT versions. If a message is missing, [ask for it](https://support.edifabric.com/hc/en-us/requests/new).

- [NCPDP SCRIPT 10.6](https://support.edifabric.com/hc/en-us/articles/360017128397-NCPDP-SCRIPT-Version-10-6)
